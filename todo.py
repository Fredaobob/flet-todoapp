...
class Task(ft.UserControl):
    ...
    def build(self):
        ...
        self.task_view = ft.Row(
            visible=True,
            controls=[
                self.task_cb, ft.IconButton(icon=ft.icons.CREATE_OUTLINED, on_click=self.edit_clicked),
...
class ToDo(ft.UserControl):
    def build(self):
        self.input = ft.TextField(hint_text="O que precisa ser feito?", expand=True)
        self.tasks = ft.Column()

        view = ft.Column(horizontal_alignment=ft.CrossAxisAlignment.CENTER, 
                         controls= [
                             ft.Text(value='Lista de afazeres', theme_style=ft.TextThemeStyle.HEADLINE_MEDIUM),
                             ft.Row(
                                 controls= [
                                     self.input,
